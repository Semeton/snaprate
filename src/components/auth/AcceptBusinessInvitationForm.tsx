...
export default function AcceptBusinessInvitationForm() {
  ...
  const [invitation, setInvitation] = useState<BusinessInvitationData | null>(
    null,
  );
  ...
  const fetchInvitation = async () => {
    ...
    try {
      const response = await fetch(
        `/api/business/accept-invitation?token=${token}`,
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const response = await fetch("/api/business/accept-invitation", {
        method: "POST",
        headers: {
